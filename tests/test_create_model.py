...
from cpvt_website_models.models import Individual, IndividualSex, Publication, \
    IndividualToPublication, Condition, IndividualCondition, \
    FamilyHistoryRecord, KinshipName, FamilyMemberHistory, Treatment, \
    IndividualOriginalExcelRow, TreatmentRecord, Variant, VariantInheritance, \
    Zygosity, IndividualVariant, VariantsDataset, DatasetVariant, \
    VariantClinVarInfo, ClinicalSignificance, ClinVarVariantLinkedCondition, \
    EditType, SequenceVariantDb, edit_type_ids, ProteinConsequence
...
def _protein_sequence_from_variant():
    sv = SequenceVariantDb(
        ...
        variant_c=parser.parse(
            "NM_001035.3(RYR2):c.14876G>A"),
...
@pytest.mark.parametrize(
    "models",
    _protein_models.values(),
    ids=_protein_models.keys()
)
def test_protein_consequence_invalid(
        models: Callable[[], ProteinConsequence]):

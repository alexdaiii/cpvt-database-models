...
def strip_nan(any_dict: dict, *, ignore_keys: set[str] = None):
    ...
    return {
        k: v for k, v in any_dict.items() if v is not None or k in ignore_keys
    }
...
class SequenceVariantDb(
            ...
            g_args = strip_nan(
                {
                    ...
                    if len(str(variant_g.posedit))
                       < ARBITRARY_MAX_VARCHAR_LENGTH
                    else None,
    ...
    def _add_edit_info(variant: SequenceVariant, all_args: dict, sv_type: str):
        ...
        if (
                ...
                and len(str(variant.posedit.edit.ref))
                < ARBITRARY_MAX_VARCHAR_LENGTH
        ):
        ...
        if (
                ...
                and len(str(variant.posedit.edit.alt))
                < ARBITRARY_MAX_VARCHAR_LENGTH
        ):
...
class ProteinConsequence(Base):
    ...
    def _create_from_sequence_variant_db(
            self, sequence_variant: SequenceVariantDb
    ):
